...
//                Log.d(TAG, "QR Code out: " + qrCode);
    ...
    private boolean detectPreviewDocument(Mat inputRgba) {
        ...
        if (quad != null) {
            ...
            double sum = Math.abs(TDFWidth) + Math.abs(TDFHeight) + Math.abs(BDFWidth) + Math.abs(BDFHeight);
            ...
            if(Math.abs(quad.points[0].x) > 20 && Math.abs(quad.points[3].x) > 20 && Math.abs(quad.points[0].y) > 20 && Math.abs(quad.points[3].y) > 20) {
                if (onceAutoTake && Math.abs(TDFWidth) < 20 && Math.abs(TDFHeight) < 20 && Math.abs(BDFWidth) < 20 && Math.abs(BDFHeight) < 30 && sum < 65) {
    ...
    private void drawDocumentBox(Point[] points, Size stdSize) {
        ...
        border.setColor(Color.rgb(255, 255, 0));
...
//            Imgproc.adaptiveThreshold(mask, mask, 255, Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C, Imgproc.THRESH_BINARY_INV, 15, 15);
