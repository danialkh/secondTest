package ir.notopia.android.scanner.liveedgedetection.view;
...
import ir.notopia.android.scanner.liveedgedetection.interfaces.IScanner;
import ir.notopia.android.scanner.liveedgedetection.util.ImageDetectionProperties;
import ir.notopia.android.scanner.liveedgedetection.util.ScanUtils;

import static ir.notopia.android.scanner.liveedgedetection.util.ScanUtils.createImageFile;
...
public class ScanSurfaceView extends FrameLayout implements SurfaceHolder.Callback {
    ...
    private final Camera.PictureCallback pictureCallback = new Camera.PictureCallback() {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            ...
            try {
                mFileOutputStream = new FileOutputStream(createImageFile("main_image"));
            } catch (IOException e) {
                e.printStackTrace();
            }
...
//        List<Camera.Size> allSizes = parameters.getSupportedPictureSizes();
        ...
        Camera.Size camSize = allSizes.get(0); // get top size
        for (int i = 0; i < allSizes.size(); i++) {
            Log.i(TAG, "CAMERASCREEN: camSize " + " width: " + allSizes.get(i).width + " height: " + allSizes.get(i).height);

            if (allSizes.get(i).width > camSize.width)
                camSize = allSizes.get(i);
        }
...
//        parameters.setPictureSize(1920, 1080);
        ...
        Log.i(TAG, "CAMERASCREEN: " + " width: " + size.width + " height: " + size.height);
        Log.i(TAG, "CAMERASCREEN: camSize " + " width: " + camSize.width + " height: " + camSize.height);
    ...
    private void autoCapture(ScanHint scanHint) {
        ...
        if (ScanHint.CAPTURING_IMAGE.equals(scanHint)) {
            try {
                ...
                Log.i(TAG, "autoCapture: " + pictureCallback.toString());
                Log.i(TAG, "autoCapture: " + mShutterCallBack.toString());
//                String pathOrginal = ScanUtils.saveToExternalStorage(pictureCallback., "debug","pictureCallback" + ".png", context, 100)[0];
                camera.setPreviewCallback(null);
//                iScanner.displayHint(ScanHint.NO_MESSAGE);
//                clearAndInvalidateCanvas();
            } catch (Exception e) {
